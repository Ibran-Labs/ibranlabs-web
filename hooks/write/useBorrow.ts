...
//0,000970198  0,0001
  ...
  // Log gas master estimation when it changes
  useEffect(() => {
    if (gasMaster !== undefined && !isLoadingGasMaster) {
      console.log("⛽ [GAS MASTER] Estimasi gas master:", {
        chainId: targetChainId,
        amount: amount || "0",
        gasMaster: gasMaster.toString(),
        gasMasterInETH: (Number(gasMaster) / 1e18).toFixed(6),
        timestamp: new Date().toISOString(),
      });
    }
  }, [gasMaster, isLoadingGasMaster, targetChainId, amount]);
  ...
  const handleBorrow = async (lendingPoolAddress: HexAddress) => {
    ...
    // Only check gas master loading if not Base (8453)
    const shouldCheckGasMasterLoading = targetChainId !== 8453;
    if (
      totalSupplyAssetsLoading ||
